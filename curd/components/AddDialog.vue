...
import { AddReplaceMeDto, UpdateReplaceMeDto, ReplaceMe } from '@/api/types/replaceMe'
import { computed, defineProps, PropType, ref, watch } from 'vue'
import { useMessage } from '@/hooks/message'
import { Form } from '@arco-design/web-vue'
import { useReplaceMeStore } from '@/stores/replaceMes'
const { addReplaceMeAction, updateReplaceMeAction } = useReplaceMeStore()
const props = defineProps({
  data: {
    type: Object as PropType<UpdateReplaceMeDto>,
    default: null,
  },
  visible: {
    type: Boolean,
...
const formData = ref<AddReplaceMeDto>({
  name: '',
})
...
const handleBeforeOk = async () => {
  ...
  try {
    if (props.data) {
      //编辑
      await updateReplaceMeAction(props.data.id, formData.value)
      message.success('编辑成功')
      return true
    }
    await addReplaceMeAction(formData.value)
